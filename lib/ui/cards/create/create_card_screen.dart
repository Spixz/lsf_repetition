...
class _CreateCardScreenState extends ConsumerState<CreateCardScreen> {
  ...
  @override
  void initState() {
    onCardsCreatedSubscription = widget.viewModel.createCards.results.listen(
      _onCardsCreated,
    );
    super.initState();
  }
  ...
  void _onCardsCreated(
    CommandResult<void, Result<void>> result,
    ListenableSubscription _,
  ) {
    if (result.hasError) {
...
class _FAB extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final CreateDeckWidgetViewmodel deckViewModel = CreateDeckWidgetViewmodel(
    ...
    return ValueListenableBuilder<CommandResult<void, Result<List<DeckModel>>>>(
      ...
      builder: (_, results, __) {
        if (results.isExecuting) {
          ...
          return Center(child: Text(context.tr("AnErrorHasOccured")));
